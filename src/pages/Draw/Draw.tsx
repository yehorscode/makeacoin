...
import { storage, coin_bucket, tablesDB } from "@/components/appwrite";
import { databaseId, collectionId } from "@/components/appwrite";
import { getUserID, getUsername } from "@/components/appwrite";
...
export default function DrawPage() {
    ...
    try {
      const uploads = await Promise.all(
        COIN_FACES.map(async (face) => {
          const canvas = canvasesRef.current[face];
          if (!canvas) {
            throw new Error(`Canvas not found for ${face} face.`);
          }
          ...
          const blob = await new Promise<Blob>((resolve, reject) => {
            canvas.toBlob((createdBlob) => {
              if (createdBlob) {
                ...
                reject(new Error(`Failed to create image blob for ${face}.`));
              }
            }, "image/png");
          });
          ...
          const fileResponse = await storage.createFile({
            ...
            fileId: ID.unique(),
            file: new File([blob], `coin-${face}-${Date.now()}.png`, {
              type: "image/png",
            }),
          ...
          const fileUrl = String(
            storage.getFileView(coin_bucket, fileResponse.$id)
          );
      ...
      const frontUpload = uploads.find((upload) => upload.face === "front");
      const backUpload = uploads.find((upload) => upload.face === "back");

      if (!frontUpload || !backUpload) {
        throw new Error("One or both coin faces failed to upload.");
      }

      const trimmedTitle = coinTitle.trim();
      const trimmedUsername = coinUsername.trim();
      ...
      await tablesDB.createRow({
        ...
        rowId: ID.unique(),
        data: {
          coin_name: trimmedTitle || "Untitled Coin",
          username: trimmedUsername || username || "Anonymous",
          front_id: frontUpload.url,
          back_id: backUpload.url,
        },
      });
      ...
      console.error("Failed to upload coin:", error);
      toast.error("Failed to upload coin. Please try again.");
    } finally {
  ...
  function openUploadingDialog() {
    if (!coinUsername && username) {
      setCoinUsername(username);
    }
    setDialogOpen(true);
  }
  ...
  return (
    <div className="flex flex-col gap-[25px] p-[25px]">
      ...
      <span className="font-mono opacity-70">
        Note: Please draw inside the circle. Why? End result will be cropped to
        the circle anything past the circle{" "}
      </span>
